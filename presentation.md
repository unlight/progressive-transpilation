...
Ну мы можем ограничить набор фич, скажем мы используем только ES6, ES7+ async / await не используем, и тогда рано или поздно мы от транспайлинга избавимся.
...
```
???
Этот eval надо обернуть в try catch, если падаем в catch, и это означает что этот браузер не смог распарсить эту строку,
...
???
Результаты измерений. Нам говорили, что размер должен уменьшится на 20% и более процентов.
...
Может быть можно еще было выиграть в размере, если бы пересобрать все зависимости в es6 из исходников, но я этим не стал заниматься.
---
# Other methods
...
у нас будет ошибка в runtime, поэтому за этим надо следить либо как-то это автоматизировать (сбор фич используемых в коде).  
